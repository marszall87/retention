...
const renderCell = ({ rangeIndex, rangeCount }, content) => {
    ...
    return html`<td class="${style.cell}" style="${{ backgroundColor: bg }}">${content}</td>`;
};
...
export default ({ container, matrix, colorPattern = defaultPattern }) => {
        ...
        <table class="${style.table}">
            <thead>
                <tr>
                    ...
                    ${matrix.map((activity, index) => renderHeader(`${ordinal(index + 1)} week`))}
                </tr>
            </thead>
